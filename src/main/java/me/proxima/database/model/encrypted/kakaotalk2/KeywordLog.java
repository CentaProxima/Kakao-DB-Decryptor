...
import me.proxima.database.model.decrypted.kakaotalk2.DecryptedKeywordLog;
import me.proxima.database.model.encrypted.Table;

//completed
@DatabaseTable(tableName = "keyword_log")
public class KeywordLog extends Table<KeywordLog, DecryptedKeywordLog> {
    @Getter @Setter
    ...
    @Override
    public DecryptedKeywordLog decrypt() {
        DecryptedKeywordLog dKeywordLog = new DecryptedKeywordLog();
        dKeywordLog.setId(this.id);
        dKeywordLog.setChatId(this.chatId);
        dKeywordLog.setCreatedAt(this.createdAt);
        dKeywordLog.setProtected(this.Protected);
        return dKeywordLog;
    }
}